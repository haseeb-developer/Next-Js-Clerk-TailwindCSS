...
export default function ConfirmAuthPage() {
  ...
  const checkPinExists = async () => {
    try {
      const pinData = (user?.unsafeMetadata as UserMetadata)?.passwordPin;
  ...
  const verifyPin = async () => {
    ...
    try {
      ...
      // Redirect to safe-passwords with PIN verification flag and session timeout
      console.log('PIN verified successfully, redirecting to safe-passwords...');
      router.push(`/safe-passwords?pinVerified=true&timeout=${sessionTimeout / 60000}`);
      console.log('Redirect command sent');
      
      // Fallback redirect in case router.push doesn't work
      setTimeout(() => {
        console.log('Fallback redirect triggered');
        window.location.href = `/safe-passwords?pinVerified=true&timeout=${sessionTimeout / 60000}`;
      }, 1000);
    } catch (error) {
          ...
          <div className="space-y-3">
            ...
            >
              {isLoading ? (
                ...
                pinExists ? 'Verify & Access' : 'Create PIN & Continue'
              )}
            </button>
